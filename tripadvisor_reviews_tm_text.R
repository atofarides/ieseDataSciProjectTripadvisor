...
top_words_plot <- ggplot(top_words_no_doc_id,aes(x=reorder(row.names(top_words_no_doc_id),.),y=.))+
  ...
  ggsave(file.path("~","GitHub","ieseDataSciProjectTripadvisor","top_terms_in_text.pdf"),
...
#Plot
top_score_plot_q1 <- ggplot(tripadvisor_reviews_q1,aes(x=reorder(row.names(tripadvisor_reviews_q1),.),y=.))+
  geom_col() +
  coord_flip(ylim = c(3.4,5)) +
  labs(title = "Top words from review text, scored by review ratings",y="Score",x="Top words") +
  ggsave(file.path("~","GitHub","ieseDataSciProjectTripadvisor","top_scoring_terms_in_text.pdf"),
         width=30,height=15,units="cm")
top_score_plot_q1
...
term_counts_q2 <- tripadvisor_reviews_tm[,c(3,8:57)] %>%
  group_by(restaurant_name) %>%
...
top_score_plot_q2 <- ggplot(tripadvisor_reviews_q2_top,aes(x=term,y=score))+
  ...
  labs(title = "Top words from review text, scored by review ratings, for each restaurant",y="Score",x="Top words") +
  ggsave(file.path("~","GitHub","ieseDataSciProjectTripadvisor","top_scoring_restaurant_terms_in_text.pdf"),
...
bottom_score_plot_q2 <- ggplot(tripadvisor_reviews_q2_bottom,aes(x=term,y=score))+
  ...
  coord_flip() + 
  facet_wrap(~restaurant_name, scales = "free") +
  labs(title = "Bottom words from review text, scored by review ratings, for each restaurant",y="Score",x="Bottom words") +
  ggsave(file.path("~","GitHub","ieseDataSciProjectTripadvisor","bottom_scoring_restaurant_terms_in_text.pdf"),
         width=40,height=23,units="cm")
